...
class Runner:
    def __init__(self, conf_path, mode='train', is_continue=False,
        ...
        # Load checkpoint
        latest_model_name = self.conf['model.ckpt']
    ...
    def train(self):
        ...
        for i in tqdm(range(length)):
            ...
            for key, value in losses_lod0.items():
                if key not in acc_losses:
                    acc_losses[key] = 0
                acc_losses[key] += value
            ...
            print(f'Current Mean Loss: {loss / num}')

            with open(os.path.join(self.base_exp_dir, "test.txt"), 'a', encoding='utf-8') as fw:
                fw.write(f"Step: {i}, Batch num: {num}:\n")
                for key, value in acc_losses.items():
                    if key != 'weights_sum' and key != 'weights_sum_fg':
                        fw.write(f"{key}: {value / num}\n")
                fw.write("\n")

            # loss /= len(dataloader)
            # print(f'Mean Loss: {loss/num}')
